...
class PropertyInitializationInfoCollector(
    ...
    override fun visitVariableDeclarationNode(
        ...
        return processVariableWithAssignment(
            dataForNode,
            node.fir.symbol,
            reset = node.fir.initializer == null && node.fir.delegate == null
        )
    }
    ...
    object CapturingStatementVisitor {
    ...
    private fun enterCapturingStatement(statement: FirStatement): Set<FirPropertySymbol> =
        CapturingStatementVisitor.enterCapturingStatement(statement)

    private fun exitCapturingStatement(statement: FirStatement) {
        CapturingStatementVisitor.exitCapturingStatement(statement)
    }
    ...
    override fun visitLoopConditionEnterNode(
        ...
        val declaredVariableSymbolsInLoop = CapturingStatementVisitor.declaredVariablesPerElement[node.loop]
        if (declaredVariableSymbolsInLoop.isEmpty())
            return visitNode(node, data)
    ...
    override fun visitLoopBlockEnterNode(
        ...
        val declaredVariableSymbolsInLoop = CapturingStatementVisitor.declaredVariablesPerElement[node.fir]
        if (declaredVariableSymbolsInLoop.isEmpty())
            return visitNode(node, data)
...
internal fun <P : PathAwareControlFlowInfo<P, S>, S : ControlFlowInfo<S, K, EventOccurrencesRange>, K : Any> overwriteRange(
    ...
    // after (if key is p1):
    //   { |-> { p1 |-> r }, l1 |-> { p1 |-> r, p2 |-> PI2 } }
    return updateRange(pathAwareInfo, key, { _ -> range }, constructor)
}
